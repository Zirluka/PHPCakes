...
if ($token) {
	// Если токен есть, то ищём имя пользователя, для последующего вывода
...
<head>
	...
	<!-- $title передаётся на самих страницах -->
	<title><?= $title ?></title>
...
<body>
	...
	<header>
		...
		<nav>
			<!-- Навигация -->
			<!-- В навигации проверяем $current_page (текущая страница) 
				 Если проверка прошла, делаем ссылку активной с помощью класса .active  -->
			...
			<!-- Если токена нету, выводим ссылки на регистрацию и авторизацию,
			Иначе выводим корзину, имя пользователя и кнопку выйти -->
			<?php if(!$token): ?>
